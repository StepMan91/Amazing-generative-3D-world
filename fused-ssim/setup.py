...
def configure_cuda():
    log("Compiling for CUDA (forced sm_90).")
    compiler_args = {
        "cxx": ["-O3", "-DFUSED_SSIM_CUDA"],
        "nvcc": ["-O3", "-DFUSED_SSIM_CUDA", "--maxrregcount=32", "--use_fast_math", "-gencode=arch=compute_90,code=sm_90"]
    }
    return CUDAExtension, ["ssim.cu", "ssim3d.cu", "ext.cpp"], "fused_ssim_cuda", compiler_args, [], "sm_90"
